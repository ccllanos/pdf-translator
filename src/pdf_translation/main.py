...
def main():
    ...
    print(Fore.CYAN + "="*65)
    print(Style.BRIGHT + " PDF TRANSLATOR V1.2 - RUTEO HÍBRIDO (LLM/GOOGLE)")
    ...
    for page_num in range(1, total_pages + 1):
        if page_num in config["selected_pages"]:
            ...
            mode_config = config["page_modes"].get(page_num, {"mode": "standard", "engine": "llm", "bg_path": None})
            page_blocks = [b for b in analyzer.elements if b.page_num == page_num]
            translated_blocks = []

            for idx, block in enumerate(page_blocks):
                print(f" -> Traduciendo bloque {idx+1}/{len(page_blocks)} [Motor: {mode_config['engine']}]...")
                
                # INVOCAMOS EL MOTOR SELECCIONADO
                resultado = translator.translate_block(
                    block.text, "English", "Spanish", engine=mode_config["engine"]
                )
            ...
            cache.save_page_translation(page_num, mode_config["mode"], mode_config["bg_path"], mode_config["engine"], translated_blocks)
